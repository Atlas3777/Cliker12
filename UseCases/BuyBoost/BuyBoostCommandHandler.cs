...
public class BuyBoostCommandHandler : IRequestHandler<BuyBoostCommand, ScoreBoostDto>
{
    ...
    public async Task<ScoreBoostDto> Handle(BuyBoostCommand request, CancellationToken cancellationToken)
    {
        ...
        if (existingUserBoost != null)
        {
            ...
            var modif = DomainConstants.BoostCostModifier;
            if (existingUserBoost.Boost.ZoneClickMultiplayer != 0)
                modif = DomainConstants.BoostCostModifierZone;
            existingUserBoost.CurrentPrice = Convert.ToInt64(existingUserBoost.CurrentPrice * modif);
        }
        ...
        else
        {
            ...
            if (userBoost.Boost.ZoneClickMultiplayer != 0)
                userBoost.CurrentPrice = Convert.ToInt64(boost.Price * DomainConstants.BoostCostModifierZone);
        ...
        return new ScoreBoostDto
        {
            Score = new ScoreDto
            {
                ...
                MultRedZone = user.UserBoosts.GetMult()
            },
